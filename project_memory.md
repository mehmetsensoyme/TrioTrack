...
- **Finansal Ondalık Format & Canlı Maskeleme (v1.6.8 - v1.6.9):** `formatUtils.ts` ile tüm uygulama genelinde Türk Lirası formatı: binlik ayıracı `.`, ondalık ayıracı `,` ve sabit 2 kuruş ondalık basamağı (`1.000,00`, `250,50`, `0,00`). v1.6.9 ile geliştirilen `CurrencyInputField` bileşeni sayesinde kullanıcı tutarı yazarken daha klavyeden tuşlandığı anda (örneğin `54885` yazıldığında anında `54.885,00` olarak) odaktan çıkmayı beklemeden canlı maskelenir.
- **Sürüm Takibi:** Her güncellemede `APP_VERSION` (`1.6.9`, versionCode 10) ve `APP_BUILD` (`2026.09.11`) artırılarak `app.json`, `package.json`, `android/app/build.gradle` ve `version.ts` dosyaları senkronize tutulur.
