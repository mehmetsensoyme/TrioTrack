...
- **Finansal Ondalık Format & Canlı Maskeleme (v1.6.8 - v1.7.0):** `formatUtils.ts` ile tüm uygulama genelinde Türk Lirası formatı: binlik ayıracı `.`, ondalık ayıracı `,` ve sabit 2 kuruş ondalık basamağı (`1.000,00`, `250,50`, `0,00`). v1.7.0 ile geliştirilen doğal yerel `CurrencyInputField` bileşeni sayesinde kullanıcı tutarı yazarken daha klavyeden tuşlandığı anda (örneğin `54885` yazıldığında anında `54.885,00` olarak) odaktan çıkmayı veya sekme değiştirmeyi beklemeden canlı maskelenir. Gizli katman kaldırılmış olup imleç ve metin seçimi doğal Android/iOS davranışındadır.
- **Sürüm Takibi:** Her güncellemede `APP_VERSION` (`1.7.0`, versionCode 11) ve `APP_BUILD` (`2026.09.11`) artırılarak `app.json`, `package.json`, `android/app/build.gradle` ve `version.ts` dosyaları senkronize tutulur.
