...
- **Finansal Ondalık Format Standardı (v1.6.8):** `formatUtils.ts` ile tüm uygulama genelinde Türk Lirası ve uluslararası finans formatı: binlik ayıracı `.`, ondalık ayıracı `,` ve sabit 2 kuruş ondalık basamağı (`1.000,00`, `250,50`, `0,00`). Kullanıcı girdileri `parseCurrencyInput` ile virgül ve noktaya duyarlı olarak sıfır kayıpla çözümlenir.
- **Sürüm Takibi:** Her güncellemede `APP_VERSION` (`1.6.8`, versionCode 9) ve `APP_BUILD` (`2026.09.11`) artırılarak `app.json`, `package.json`, `android/app/build.gradle` ve `version.ts` dosyaları senkronize tutulur.
